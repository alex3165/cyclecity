...
class UserManager implements UserProviderInterface
{
    ...
    protected function hydrateUser(array $data)
    {
        ...
        $user->setCreationTime($data['creation_time']);
     ...
     * @return User
     */
    public function createUser($login, $plainPassword, $name = null, $roles = array())
    {
        $user = new User($login);
    ...
    {
        $sql = 'INSERT INTO users (login, password, salt, name, roles, creation_time) VALUES (:login, :password, :salt, :name, :roles, :creation_time) ';
     ...
     *
     * Invokes User::validate(), and additionally tests that the User's login isn't associated with another User.
     *
     * @param User $user
    ...
    public function validate(User $user)
    {
        ...
        $duplicates = $this->findBy(array('login' => $user->getLogin()));
        if (!empty($duplicates)) {
            foreach ($duplicates as $dup) {
                if ($user->getId() && $dup->getId() == $user->getId()) {
                    continue;
                }
                $errors['login'] = 'An account with that login already exists.';
            }
        }
